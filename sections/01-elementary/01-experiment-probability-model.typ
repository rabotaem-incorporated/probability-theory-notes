...
#notice[
    $P(B | A) + P(B | neg(A))$ не обязательно $1$. Например, в примере с кубиком, если $A = {2, 4, 6}$, а $B = {3, 6}$, то $P(B | A) = 1/3$, и $P(B | neg(A)) = 1/3$. Если взять вместо $B$, $neg(B)$, то сумма вероятностей будет больше 1.
]
...
#def[
    ...
    $
    Если домножить на знаменатель, исчезнет проблема с делением на $0$, да и формула станет симметричнее, поэтому так определение состовляют редко.
]
...
#notice[
    ...
    $
        P(A) = P(B) = P(C) = 1/2,
    $
    а следующие события, вообще одно и тоже:
    $
