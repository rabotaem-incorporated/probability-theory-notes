...
#def[
    ...
    - $P$ --- _вероятностная мера_ на $Ff$ (вероятностная, если $P(Omega) = 1$).
]
...
#def[
    Последовательность событий $A_1, A_2, ...$ называется _независимой в совокупности_, если для любого натурального $n$ и любого набора различных индексов $i_1, i_2, ..., i_n$ верно
    $
...
#lemma(name: "Борелля-Кантелли")[
    ...
    2. Если события $A_1, A_2, ...$ независимы в совокупности, и ряд $sum_(n = 1)^oo P(A_n) = +oo$ расходится, то вероятность того, что случилось бесконечное количество из событий $A_n$, равна единице.
]

#proof[
    Пусть $B$ --- событие "случилось бесконечное количетсво $A_n$". Тогда
    $
