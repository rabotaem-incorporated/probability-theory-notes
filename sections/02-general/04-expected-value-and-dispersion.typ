...
#examples[
    ...
    3. $xi sim Nn(0, 1)$.
        ...
        $
        Храбров такую базу на лекции выдал: что интеграл который получается на последней строчке можно не считать, так как это плотность какого-то распределения, и она просто равна $1$.
...
#proof[
    Пусть $M := root(5, n)$. Пусть $xi(k)$ --- количество различных простых $<= M$ в разложении $k$. Тогда $nu(k) - 4 <= xi(k) <= nu(k)$, так как если простых больше $M$ 5, то их произведение больше $n$. Будем смотреть на $xi(k)$, а не $nu(k)$, так как в любом случае они отличаются не более чем на константу, и на нашу оценку не повлияют.

    Положим
    $
        xi_p (k) = cases(
            1\, "если" k dots.v p, 0\, "иначе"
        ).
    $
    ...
    $
        P(abs(xi(k) - E xi) >= omega(n)/2 sqrt(D xi)) <= (D xi)/(omega(n)^2/2 sqrt(D xi)^2) = 4/(omega^2(n)) --> 0.
    $
]
