...
#proof[
    Рассмотрим схему Бернулли на вершинах. Если успех, то отправляем вершину в одну долю ($T$), иначе в другую ($V without T$). Рассмотрим $xi_(x y) = 1$ если $x$ и $y$ из разных долей, и $0$ иначе. Пусть
    $
...
#notice[
    ...
    1. $E (xi nu) =: dotp(xi, eta)$ --- скалярное произведение.#TODO[оговорка] Матожидание тогда --- проекция на ось констант. Действительно, $dotp(xi - E xi, a) = a E(xi - E xi) = 0$
    2. $cov(xi, nu) =: dotp(xi, eta)$ --- тоже скалярное произведение. Едиснтвенное, надо отождествить случайные величины отличающиеся на константу (так как для таких случайных величин не следует, что они равны). Тогда $sqrt(D xi)$ --- норма.
