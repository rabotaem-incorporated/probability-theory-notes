...
#proof[
    ...
    $
        E xi_i^4 &<= C,\
        E xi_i^2 &<=^"Ляпунов" sqrt(E xi_i^4) <= sqrt(C).
    $
    Значит
    $
        sum E xi_k^4 + 6 sum_(i < k) E xi_i^2 E xi_k^2 <= n C + 6 n^2 C = O(n^2).
    $
    Рассмотрим событие $A(eps) = Sect_(n = 1)^oo Union_(k = n)^oo A_k$. $P(A(eps)) = 0$ для любого $eps > 0$ (то есть то $eps$, которое мы раньше рассматривали произвольным, теперь рассматривается как параметр для множества $A$). Рассмотрим $B := Union_(m = 1)^oo A(1/m)$ и $P(B) = 0$.
