...
#th(name: "Центральная предельная теорема в форме Линденберга")[
  Пусть $xi_1$, $xi_2$, $xi_3$, ... --- независмые, $a_n := E xi_n$, $sigma_n^2 := D xi_n > 0$, $S_n := xi_1 + xi_2 + ... + xi_n$, $D_n^2 := sum_(k = 1)^n sigma_k^2 = D_(S_n)$ конечна. Тогда если для любого $eps > 0$
  $
