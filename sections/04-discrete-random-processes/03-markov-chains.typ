...
#def[
  ...
  - $a$ --- _существенное_, если для любого $b$ достжимого из $a$, есть достижимость $a$ из $b$.
]

#exercise[
  Цепь конечная. Доказать, что существует хотя бы одно существенное состояние.
]
...
#proof[
  ...
  $
    F(t) = (P(t) - 1)/(P(t)).
  $
  - Пусть $P_a = sum_(n = 1)^oo p_(a, a) (n) < +oo$. Тогда $P(t) -->_(t -> 1-) P_a + 1$ по теореме Абеля. $F(t) = (P(t) - 1)/(P(t)) -->_(t -> 1-) P_a/(P_a + 1)$, а еще $F(t) -->_(t -> 1-) F_a$. Значит они равны.
...
#th(name: "солидарности")[
  ...
  1. Либо $a$ и $b$ нулевые, либо $a$ и $b$ ненулевые.
  2. Либо $a$ и $b$ возвратные, либо $a$ и $b$ невозвратные.
]

#proof[
  1. Пусть $a$ нулевое. Тогда $p_(a, a) (n) --> 0$. $a$ и $b$ сообщающиеся, поэтому существуют $i$ и $j$ такие, что $p_(a, b) (i) > 0$, и $p_(b, a) (j) > 0$. Вероятность попать из $a$ в $a$ не меньше, чем по маршруту $a ~~> b ~~> b ~~> a$. То есть $0 <-- p_(a, a) (i + n + j) >= p_(a, b) (i) dot p_(b, b) (n) dot p_(b, a) (j)$. Это стремится к нулю только если $p_(b, b) (n) --> 0$.
