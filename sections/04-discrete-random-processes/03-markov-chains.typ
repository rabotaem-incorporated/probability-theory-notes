...
#example[
  ...
  $
    ...
    P(xi_0 = b - 1) dot 1/2 + P(xi_0 = b + 1) 1/2 =
    (pi(b - 1) + pi(b + 1))/2.
  $
  ...
  Если $c > 0$, то 
  $
    pi(n) = pi((n - 1) + (pi(n) - pi(n - 1))) = c + pi(n - 1) = ... = n c + pi (0).
  $
  Но тогда для любого $c > 0$, начиная с какого-то $n$ вероятность будет больше 1. Аналогично, если $c < 0$ (для больших $n$ она меньше $0$). Значит $c = 0$. Но это тоже дичь какая-то: $pi equiv const$, то есть во всех вещественных точках вероятность одна и та же.
]
...
#proof[
  Пусть $\#Y = m$. Рассмотрим $RR^m$ с нормой --- сумма модулей координат: $norm(x) = abs(x_1) + ... + norm(x_m)$. Рассмотрим отображение $T: RR^m --> RR^m$ вида $x arrow.bar.long (x^T P)^T = P^T x$ --- это линейное отображение. Будем смотреть на него на 
  $
