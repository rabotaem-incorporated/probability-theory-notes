...
#proof[
  1. Пусть $a$ нулевое. Тогда $p_(a, a) (n) --> 0$. $a$ и $b$ сообщающиеся, поэтому существуют $i$ и $j$ такие, что $p_(a, b) (i) > 0$, и $p_(b, a) (j) > 0$. Вероятность попать из $a$ в $a$ не меньше, чем по маршруту $a ~~> b ~~> b ~~> a$. По есть $p_(a, a) (i + n + j) >= p_(a, b) (i) dot p_(b, b) (n) dot p_(b, a) (j)$. Это равно нулю только если $p_(b, b) (n) --> 0$.
