...
#th[
  Рассмотрим произвольное симмутричное случайное блуждание по $ZZ$  (не обязательно в соседние точки). $xi_1$, $xi_2$, ... --- независимые симметричные случайные величины. $S_n = xi_1 + xi_2 + ... + xi_n$.

  Пусть $E abs(xi_1) < +oo$. Тогда такое случайное блуждание возвртано.
]

#proof[
  $p_(0, 0) (n) = P(S_n = 0)$. Распишем производящую функцию
  $
    G(z) := sum_(n = -oo)^(oo) P_(xi_1 = n) z^n.
  $
  ...
  $
  Знаем $E abs(xi_1) < +oo$, $sum_(n = -oo)^oo P(xi_1 = n) < +oo$. Что мы знаем про $G$?
  $
  ...
  $
    1/(2pi i) integral_(abs(z) = 1) (dif z)/(z (1 - x G(z))) =
    1/(2pi) integral_0^2pi (dif z)/(1 - x G(e^(i t))) =
...
#proof[
  При $d = 2$ повернем оси на 45#sym.degree. Рассмотрим $xi_n = (xi'_n, xi''_n)$ --- шаги в исходных координатах, и $eta_n = (eta'_n, eta''_n)$ --- в новых. Тогда $P(eta'_n = 1) = 1/2 = P(eta'_n = -1)$, и $P(eta''_n = 1) = 1/2 = P_n(eta''_n = -1)$. То есть теперь обе проекции смещаются, причем независимо. Тогда это просто 2 параллельных случаных блуждания по одномерной прямой. Формально,
  $
  ...
  $
    ...
    C_(2n)^n/(6^n) dot 3^n dot max_(k + j <= n) binom(n, k, j, n - k - j) approx (C^n_(2n))/(2^n) binom(n, n/3, n/3, n/3)
  $
  и применяем Стирлинга.
